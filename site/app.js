...
function render(){if(!state.feed)return;const query=$('search').value.trim().toLowerCase();const min=Number($('priority').value);const evidence=$('verification').value;const source=$('source').value;const day=$('archive-date').value;
  let rows=inView().filter(s=>(!state.topic||(s.categories||[]).includes(state.topic))&&s.importance>=min&&(!evidence||s.verification===evidence)&&(!source||s.sources.some(x=>x.name===source)||s.highlights.some(h=>'@'+h.handle===source))&&(!query||query.split(/\s+/).every(q=>searchText(s).includes(q)))&&(!(state.view==='archive'&&day)||dayKey(s.published_at)===day));
  ...
  $('empty-description').textContent=state.error?'The feed could not be loaded. Try reloading the page.':filters?'Try another company, ticker or a broader set of filters.':state.feed.generated_at?'No stories passed the relevance and attribution checks for this view.':'Stories will appear here after the first successful collection. The schedule is 05:55 UK time, Monday to Friday.';
  updateSelection();}
function metadata(){const f=state.feed,active=activeStories();$('collection-coverage').hidden=!f.partial;$('nav-total').textContent=active.length;$('nav-x').textContent=f.x_digest_ids.length;$('topic-all-count').textContent=active.length;$('metric-total').textContent=active.length;$('metric-priority').textContent=active.filter(s=>s.importance>=60).length;
...
function setView(view){state.view=view;document.querySelectorAll('[data-view]').forEach(b=>{b.classList.toggle('active',b.dataset.view===view);b.setAttribute('aria-pressed',b.dataset.view===view);});const names={all:'Morning feed',x:'X highlights',archive:'Archive'};$('breadcrumb-view').textContent=names[view];$('page-title').innerHTML=({all:'The morning brief',x:'The X shortlist',archive:'The story archive'})[view]+'<span>.</span>';$('page-description').textContent=({all:'News intelligence powered by OpenAI, xAI and Bright Data.',x:'Up to five subjects. Original reporting first, highlighting posts alongside.',archive:'Search earlier editions by company, ticker, source or date.'})[view];$('archive-date-wrap').hidden=view!=='archive';render();}
